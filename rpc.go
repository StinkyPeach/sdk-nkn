...
func httpPost(ctx context.Context, addr string, reqBody []byte, timeout time.Duration, proxy string) ([]byte, error) {
    ...
    if proxyUrl, err = url.Parse(proxy); len(proxy) != 0 && err != nil {
        log.Printf("http proxy url format error ")
        os.Exit(-1)
    }
    ...
    for i := 0; i < n; i++ {
        ...
        go func() {
            ...
            for rpcAddr := range rpcAddrChan {
                body, err := httpPost(ctx, rpcAddr, req, time.Duration(config.RPCGetRPCTimeout())*time.Millisecond, GetProxyUrl())
                if err != nil {
                    select {
                    case <-ctx.Done():
                        return
